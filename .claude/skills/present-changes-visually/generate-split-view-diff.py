...
def file_section(index: int, file: dict) -> str:
    ...
    if file["oldPath"]:
        name = f'<span class="from">{html.escape(file["oldPath"])} &rarr; </span>{name}'
    badge_class = file["statusLabel"].replace(" ", "-")
    open_attr = " open" if file["changed"] else ""
    inner = (
        f'<div class="note">{html.escape(file["note"])}</div>'
        if file.get("note")
        else '<div class="slot"><div class="pending">Loading&hellip;</div></div>'
    )
    return (
...
def index_list(files: list[dict]) -> str:
    ...
    for index, file in enumerate(files):
        ...
        label = file["path"]
        if file["oldPath"]:
            label = f'{file["oldPath"]} &rarr; {file["path"]}'
        items.append(
...
def rev_card(label: str, info: dict) -> str:
    ...
    return (
        ...
        f'<div class="subject">{html.escape(" &middot; ".join(filter(None, [info["author"], info["date"]])))}'
        f'</div></div>'
    )
...
<div class="wrap">
<header class="page">
  <h1>Comparing {html.escape(base['short'])}&hellip;{html.escape(head['short'])}
...
<div class="toolbar">
  <input id="filter" type="search" placeholder="Filter files by path&hellip;" autocomplete="off">
